...
class Dataset(object):
	...
	Args:
		attributes (list[Attributes]): The attributes of the dataset's instances.用于处理数据集（实例集）的类。
		class_index (int): The index of the dataset's class attribute. (default -1)数据集的class属性的索引。（默认值-1）
		instances (list[Instance]): A list of instances of the dataset. 数据集实例的列表。
			If not specified an empty dataset is created. (default None)如果未指定，则创建空数据集。（默认无）
		name (str): The name of the dataset. (default 'New dataset')数据集的名称，默认叫'New dataset'
	"""
