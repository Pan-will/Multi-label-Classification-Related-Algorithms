...
def calc_metrics(y_test, y_pred, row_name):
    # 调用sklearn库函数，根据预测时和真实值计算精度
    accuracy = accuracy_score(y_test, y_pred)
    # 计算精度，召回率，F1和 support for each class
    metrics = list(precision_recall_fscore_support(y_test, y_pred, average='weighted', labels=np.unique(y_pred)))
    metrics = pd.DataFrame({'accuracy': accuracy, 'precision': metrics[0], 'recall': metrics[1], 'f1': metrics[2]},
                           index=[row_name])
    return metrics
