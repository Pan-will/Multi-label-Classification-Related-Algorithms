...
# 多个预测值数组，每个预测利用投票法，得到一个预测值矩阵
def get_manyArray_majory(all_pred_array):
    final_pred = np.zeros(all_pred_array[0].shape)
    for i in range(final_pred.shape[0]):
        for j in range(final_pred.shape[1]):
            one_label_pre = []
            for index in range(len(all_pred_array)):
                one_label_pre.append(all_pred_array[index][i][j])
            y_pred_major = max(one_label_pre, key=one_label_pre.count)
            final_pred[i][j] = y_pred_major
    return final_pred


# 计算基学习器的数量，根据公式，当公式所得概率值大于参数中的概率阈值时的m，此m即为基学习器的个数
...
# 计算弱学习器特征序号,参数是弱学习器的数量，特征向量。从参数传来的特征向量（长度为n）中随机选根号n个作为基学习器的特征。
def get_baseLearner_feature_indexes(n_base_learner, feature_vector_size):
    base_trees_features = []
    for i in range(n_base_learner):
        base_trees_features.append(random.sample(range(0, feature_vector_size), int(feature_vector_size * 0.7)))
    return base_trees_features


# 将向量标准化，形参floats是list类型
