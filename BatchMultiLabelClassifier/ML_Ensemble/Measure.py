...
# 计算supervise，参数是指标名称，y_prob,标签,阈值
...
def do_metric(y_prob, label, threshold):
    ...
    accuracy = compute_accuracy(y_prob, label)
    auc = compute_auc(y_prob, label)
    precision = compute_average_precision(y_prob, label)
    # 将各个指标值用list返回
    return [hamming_loss, one_error, coverage, ranking_loss, precision, auc]